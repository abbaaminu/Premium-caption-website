...
import { Download, Sparkles, Languages, MonitorPlay, Zap, ArrowRight, Play, Pause, RefreshCw, Volume2, Cpu } from 'lucide-react';
import { RoutePath } from '../types';
import { motion } from 'motion/react';

// ─── Centralised download URL ────────────────────────────────────────────────
const DOWNLOAD_URL =
...
export default function Hero({ navigateTo }: HeroProps) {
  ...
  const handleDownload = (e: React.MouseEvent) => {
    e.preventDefault();
    // Only Windows build available — show Coming Soon for Mac/Linux
    if (selectedOS !== 'win') {
      alert("macOS and Linux builds are coming soon! Only the Windows installer is available right now.");
      return;
    }
    setDownloading(true);
    setTimeout(() => {
      ...
      // Direct OneDrive download — no scan warning
        ...
        <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-12 lg:gap-16">
          ...
          <div className="text-center lg:col-span-5 lg:text-left">
            ...
            <h1 className="mt-6 font-display text-4xl font-extrabold tracking-tight text-gray-900 dark:text-white sm:text-5xl lg:text-5xl lg:leading-[1.15]">
              ...
              <span className="text-gradient">VLC & MPV</span>{' '}
              Offline
            </h1>
            ...
            <div className="mt-8 flex items-center justify-center lg:justify-start gap-1">
              ...
              {(['win', 'mac', 'linux'] as const).map((os) => (
                <button key={os} onClick={() => setSelectedOS(os)}
                  className={`px-3 py-1 text-xs rounded-md font-medium transition-all ${selectedOS === os ? 'bg-sky-100 text-sky-800 dark:bg-white/10 dark:text-sky-300 ring-1 ring-sky-400/30' : 'text-gray-500 hover:text-gray-900 dark:hover:text-gray-200 dark:text-slate-400'}`}>
                  {os === 'win' ? 'Windows (x64)' : os === 'mac' ? 'macOS (Apple Silicon)' : 'Linux (.AppImage)'}
                </button>
              ))}
            </div>
            ...
            <div className="mt-4 flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4">
              <button onClick={handleDownload} disabled={downloading}
                ...
                {downloading ? (
                  <><RefreshCw className="h-5 w-5 animate-spin" /><span>Preparing download...</span></>
                ) : (
            ...
            {downloadSuccess && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                ...
                <span>Download started! Run the installer and enjoy 30 minutes free, then subscribe for $1/month.</span>
              </motion.div>
            )}
            ...
            <div className="mt-8 flex flex-col gap-2 border-t border-gray-200/85 pt-6 dark:border-white/10">
              <div className="flex items-center justify-center lg:justify-start gap-4 text-xs font-mono text-gray-500 dark:text-slate-400">
                <span className="flex items-center gap-1"><Cpu className="h-3.5 w-3.5 text-sky-400" />100% Offline Speech tracking</span>
                <span>•</span>
                <span className="flex items-center gap-1"><Languages className="h-3.5 w-3.5 text-indigo-400" />No Cloud Data Pipelines</span>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-7">
            <div className="relative rounded-2xl border border-gray-200 bg-white p-2 shadow-2xl dark:border-white/10 dark:bg-white/5 glow-blue" id="pyside-mockup-container">
              <div className="flex items-center justify-between border-b border-gray-100 bg-gray-50/50 px-4 py-2.5 dark:border-white/10 dark:bg-[#0F172A]/50 rounded-t-xl">
                ...
                <span className="rounded-md accent-gradient px-2 py-0.5 font-mono text-[10px] font-bold text-white shadow-sm shadow-sky-500/10">VLC Sync Pipeline Active</span>
              </div>
              ...
              <div className="bg-gray-950 p-4 rounded-b-xl aspect-video relative overflow-hidden flex flex-col justify-between">
                <div className="flex items-center justify-between z-10 bg-black/45 backdrop-blur-xs p-2 rounded-lg border border-white/5">
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={() => { setActiveVideoIdx((prev) => (prev + 1) % MOCK_VIDEOS.length); setCurrentLineIdx(0); }}
                      className="text-[10px] bg-white/10 hover:bg-white/20 text-white rounded px-2 py-1 font-mono flex items-center gap-1 transition-all" id="mockup-switch-video">
                      <RefreshCw className="h-2.5 w-2.5" />Switch File
                    </button>
                ...
                <div className="my-auto flex flex-col items-center justify-center p-6 text-center z-10">
                  ...
                  <div className="bg-black/80 backdrop-blur-md rounded-xl p-4 border border-white/10 shadow-lg max-w-lg w-full transition-all duration-300 min-h-[90px] flex flex-col justify-center">
                    <p className="text-sm font-medium text-white mb-2 leading-relaxed">"{activeVideo.speechLines[currentLineIdx].text}"</p>
                    <div className="pt-2 border-t border-white/10 flex items-center gap-1.5 justify-center">
                      ...
                      <p className="text-xs font-semibold text-sky-300 leading-relaxed">{activeVideo.speechLines[currentLineIdx].translation}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-auto bg-black/60 backdrop-blur-xs p-2 rounded-lg border border-white/5 flex items-center justify-between z-10">
                  <div className="flex items-center gap-2">
                    <button onClick={() => setIsPlaying(!isPlaying)} className="p-1 rounded accent-gradient text-white transition-all focus:outline-none" id="mockup-play-toggle">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="flex items-center gap-1">
                      ...
                      <button onClick={() => setTargetLang('es')} className={`text-[10px] px-2 py-0.5 rounded font-bold font-mono transition-all ${targetLang === 'es' ? 'bg-sky-500 text-white' : 'bg-white/10 text-gray-300 hover:bg-white/15'}`} id="target-es">ES (Spanish)</button>
                      <button onClick={() => setTargetLang('en')} className={`text-[10px] px-2 py-0.5 rounded font-bold font-mono transition-all ${targetLang === 'en' ? 'bg-sky-500 text-white' : 'bg-white/10 text-gray-300 hover:bg-white/15'}`} id="target-en">EN (Original)</button>
                    </div>
                    <div className="flex items-center gap-1 text-gray-400">
                      <Volume2 className="h-3.5 w-3.5" />
                      <div className="w-12 h-1 bg-white/20 rounded"><div className="w-9/12 h-full bg-sky-500 rounded" /></div>
                    </div>
                  </div>
                </div>
